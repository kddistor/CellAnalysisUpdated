...
% 
% 
gapCloseParam.timeWindow = 3; %def: 7. maximum allowed time gap (in frames) between a track segment end and a track segment start that allows linking them.
...
parameters.minSearchRadius = 2; %def: 2. minimum allowed search radius. The search radius is calculated on the spot in the code given a feature's motion parameters. If it happens to be smaller than this minimum, it will be increased to the minimum.
parameters.maxSearchRadius = 7; %def: 7. maximum allowed search radius. Again, if a feature's calculated search radius is larger than this maximum, it will be reduced to this maximum.
...
parameters.minSearchRadius = 2; %def: 2. minimum allowed search radius.
parameters.maxSearchRadius = 7; %def: 7. maximum allowed search radius.
...
parameters.maxAngleVV = 30; %def: 30.maximum angle between the directions of motion of two tracks that allows linking them (and thus closing a gap). Think of it as the equivalent of a searchRadius but for angles.
